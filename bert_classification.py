...
def create_balanced_train_test_sets(dataset, test_size=100):
    data_per_emoji = {emoji: [] for data, emoji in dataset}
    for text, emoji in dataset:
        data_per_emoji[emoji].append(text)

    test = {emoji: [textlist.pop(randrange(len(textlist))) for _ in range(test_size)] for emoji, textlist in data_per_emoji.items()}
...
def main():
    ...
    train, test = create_balanced_train_test_sets(data)
    x_train, y_train = split_x_y(train)
    x_test, y_test = split_x_y(test)

    tknzr = get_tokenizer()
    lblmp = get_labelmap(y_train)

    print("%s:      Tokenizing training data" % datetime.now())
    id_train, mask_train, segment_train = tokenize_inputlist(x_train, tknzr)
    label_train = tokenize_labels(y_train, lblmp)
    ...
    with open('otherdata', 'wb') as file:
    ...
    while another_epoch:
        ...
        inverselabelmap = {value: key for key, value in lblmp.items()}
        ...
        another_epoch = bool(int(input('Another epoch?')))

    print("%s:      Saving model" % datetime.now())
    model.save('Trained_BERT.h5')
